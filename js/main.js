...
class UIManager {
  ...
  handleAddTodo() {
    ...
    if (task === "") {
        ...
        // Check if the due date is within the current time
        ...
        if (taskDueTime > currentTime) {
            ...
            // Set a timeout to trigger the notification and sound
            setTimeout(() => {
                this.triggerPushNotification(newTodo.task);
            }, timeDifference);
        }
    }
}
...
triggerPushNotification(task) {
    if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('Task Due', { body: task });
